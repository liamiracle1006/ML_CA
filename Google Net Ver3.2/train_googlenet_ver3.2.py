"""
Train GoogLeNet - Version 3.2
Strategy: Higher Learning Rate + Light Regularization
- LR: 0.002 (2x higher)
- Dropout: 0.3 (lighter)
- Weight Decay: 0.01 (lighter)
- Goal: Faster convergence with less constraint
"""
import torch
...
class GoogLeNet(nn.Module):
    def __init__(self, num_classes=4):
        ...
        self.dropout = nn.Dropout(0.3)  # VERSION 3.2: Lighter dropout
...
def plot_confusion_matrix(y_true, y_pred, class_names, model_name, save_path):
    ...
    plt.title(f'{model_name} - Confusion Matrix (v3.2)', fontsize=16, fontweight='bold', pad=20)
...
def train_model(model_name, model, num_epochs=NUM_EPOCHS):
    print(f"\n{'='*60}")
    print(f"Training: {model_name} (Version 3.2)")
    print(f"{'='*60}")
    ...
    # VERSION 3.2: Higher LR + lighter weight decay
    ...
    print(f"Loading best model from best_{model_name}_v3.2.pth...")
    checkpoint = torch.load(f'best_{model_name}_v3.2.pth')
    ...
    plot_confusion_matrix(y_true, y_pred, full_train_dataset.classes, model_name,
                         f'{model_name}_confusion_matrix_v3.2.png')
    ...
    ax1.set_title(f'{model_name} - Training Loss (v3.2)', fontweight='bold')
    ...
    ax2.set_title(f'{model_name} - Accuracy (v3.2)', fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig(f'{model_name}_training_v3.2.png', dpi=300, bbox_inches='tight')
    print(f"\n[SAVED] {model_name}_training_v3.2.png")
    plt.close()
...
print("\n" + "="*60)
print("TRAINING GOOGLENET - VERSION 3.2")
print("Higher LR + Light Regularization")
print("="*60)
...
print("\n" + "="*80)
print("TRAINING COMPLETED - VERSION 3.2")
print("="*80)

print(f"\n{result['model_name']} (v3.2):")
...
with open('googlenet_results_v3.2.json', 'w') as f:
    json.dump(result, f, indent=2)
print("\n[SAVED] googlenet_results_v3.2.json")

print("\n" + "="*80)
print("DONE! Check GoogLeNet_training_v3.2.png for training curves.")
print("="*80)
