...
def plot_confusion_matrix(y_true, y_pred, class_names, model_name, save_path):
    ...
    plt.title(f'{model_name} - Confusion Matrix (v3.6)', fontsize=16, fontweight='bold', pad=20)
...
def train_model(model_name, model, num_epochs=NUM_EPOCHS):
    print(f"\n{'='*60}")
    print(f"Training: {model_name} (Version 3.6)")
    print(f"{'='*60}")
    ...
    # VERSION 3.6: Classical SGD with momentum instead of AdamW
    optimizer = optim.SGD(model.parameters(), lr=LEARNING_RATE, momentum=MOMENTUM, weight_decay=0.0001)

    # VERSION 3.6: StepLR instead of CosineAnnealingLR (decay by 0.1 every 15 epochs)
    ...
    print(f"Loading best model from best_{model_name}_v3.6.pth...")
    checkpoint = torch.load(f'best_{model_name}_v3.6.pth')
    ...
    plot_confusion_matrix(y_true, y_pred, full_train_dataset.classes, model_name,
                         f'{model_name}_confusion_matrix_v3.6.png')
    ...
    ax1.set_title(f'{model_name} - Training Loss (v3.6)', fontweight='bold')
    ...
    ax2.set_title(f'{model_name} - Accuracy (v3.6)', fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig(f'{model_name}_training_v3.6.png', dpi=300, bbox_inches='tight')
    print(f"\n[SAVED] {model_name}_training_v3.6.png")
    plt.close()
...
print("\n" + "="*60)
print("TRAINING GOOGLENET - VERSION 3.6")
...
print("\n" + "="*80)
print("TRAINING COMPLETED - VERSION 3.6")
print("="*80)

print(f"\n{result['model_name']} (v3.6):")
...
with open('googlenet_results_v3.6.json', 'w') as f:
    json.dump(result, f, indent=2)
print("\n[SAVED] googlenet_results_v3.6.json")

print("\n" + "="*80)
print("DONE! Check GoogLeNet_training_v3.6.png for training curves.")
print("="*80)
